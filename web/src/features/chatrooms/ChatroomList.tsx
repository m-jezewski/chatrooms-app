      ...
      {chatrooms.map((chatroom) => (
        <Link to={`chatrooms/${chatroom.id}`} key={chatroom.id}>
          <div
            ...
            className={[
              activeChatClass(chatroom.id),
              'flex text-wrap break-all flex-col gap-3 w-full mt-2 p-1 font-normal transition-all hover:bg-white/5 rounded  text-white border-b border-neutral-700',
            ].join(' ')}>
            # {chatroom.name}
          </div>
        </Link>
