...
const SidebarContent = ({onLogout, onAddChatroom, onNavigate}: {
    onLogout: () => void;
    onAddChatroom: () => void;
    onNavigate?: () => void;
}) => {
        ...
        <>
            {user?.role === "ADMIN" && (
                <Link to="/dashboard" onClick={onNavigate}>
                    <div className="text-center p-2 bg-white/5">Users</div>
                </Link>
            )}
            <AppButton onClick={onLogout}>Logout</AppButton>
            <AppButton variant={'purple'} onClick={onAddChatroom}>Add chatroom</AppButton>
            <ChatroomList/>
        </>
    );
};
        ...
        <div className={"flex flex-row items-stretch h-screen w-screen bg-gradient-to-tr from-slate-900 to-neutral-900 relative"}>
            ...
            {mobileMenuOpen && (
                <div className="absolute top-0 left-0 w-full h-full bg-slate-900 bg-opacity-95 z-50 flex flex-col gap-4 p-6">
                    ...
                    <SidebarContent
                        onLogout={() => { logout(); closeMobile(); }}
                        onAddChatroom={() => { setIsOpen(true); closeMobile(); }}
